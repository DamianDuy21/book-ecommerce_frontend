...
const ProductDetailPage = () => {
        ...
        const fetchProduct = async () => {
            try {
                ...
                if (response && response.data && response.data.data.length > 0) {
                    ...
                    const relatedDataFilter = allData.filter(item => {
                        if (item._id !== param.id) {
                            return item.category.some(category => categoryArray.includes(category));
                        }
    ...
    useEffect(() => {
        const handleResize = () => {
            const paymentSecLE1200 = document.querySelector("[data-payment-section-le1200]");
            const paymentSecGE1200 = document.querySelector("[data-payment-section-ge1200]");

            if (paymentSecLE1200 && paymentSecGE1200) { // Check if elements are not null
    ...
    return (
        <div className="container">
            ...
            <div className="slider-list-cart common-wrapper">
                <SliderList products={relatedProducts} title={"Có thể bạn quan tâm"} related={param.id} />
            </div>
        </div>
    );
};
