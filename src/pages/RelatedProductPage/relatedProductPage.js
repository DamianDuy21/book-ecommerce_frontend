...
const RelatedProductPage = () => {
    ...
    const fetchData = async () => {
        ...
        if (responeMainProduct) {
            setMainProduct(responeMainProduct.data.data[0])
        }
        let response = null;
        response = await getProductByRequest(`price_gte=1&price_lte=999999999&category=${responeMainProduct.data.data[0].category}`);
        if (response) {
            ...
            <>
                <div className="container">
                    <div className="common-wrapper" style={{ marginBottom: "16px" }}>
                        <div className="medium-title" style={{ marginBottom: "0px" }}>Liên quan tới: {mainProduct.name}</div>
                    </div>
